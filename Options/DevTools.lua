        ...
        for _, treeID in ipairs( configInfo.treeIDs or {} ) do
            ...
            for _, nodeID in ipairs( C_Traits.GetTreeNodes( treeID ) or {} ) do
                ...
                if nodeInfo and nodeInfo.maxRanks > 0 then
                    ...
                    if nodeInfo.subTreeID then
                        ...
                        if subTreeInfo then
                            ...
                            if id == classCurrencyID then
                                isClass, treeName = true, "Class"
                            elseif id == specCurrencyID then
                                isSpec, treeName = true, specName
                            elseif validHeroTrees[ nodeInfo.subTreeID ] then
                                isHero, treeName = true, subTreeInfo.name
                            end
                        end
                    end

                    if not ( isClass or isSpec or isHero ) then
                        for _, cost in ipairs( C_Traits.GetNodeCost( configID, nodeID ) or {} ) do
                            if cost.ID == classCurrencyID then isClass, treeName = true, "Class"
                            elseif cost.ID == specCurrencyID then isSpec, treeName = true, specName end
                        end
                    end
                    ...
                    if isHero and not validHeroTrees[ nodeInfo.subTreeID ] then
                        isHero = false
                    end
...
function SkeletonGen:Generate()
    ...
    self:increaseIndent()
    for k, v in orderedPairs( self.talents ) do
    ...
    self:increaseIndent()
    for k, aura in orderedPairs( self.auras ) do
        self:append( string.format( "-- https://www.wowhead.com/spell=%d", aura.id ) )
        if aura.tooltip and aura.tooltip ~= "" then
            self:append( "-- " .. aura.tooltip )
        end
        self:append( k .. " = {" )
        self:increaseIndent()
        self:append( "id = " .. aura.id .. "," )
        self:append( "duration = " .. ( aura.duration or 0 ) .. "," )
        if aura.type and aura.type ~= "None" then
            self:append( "type = \"" .. aura.type .. "\"," )
        end
        if aura.max_stack then
            self:append( "max_stack = " .. aura.max_stack .. "," )
        end
        self:decreaseIndent()
        self:append( "}," )
    end
