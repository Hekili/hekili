...
do
    ...
    spec:RegisterEvent( "PLAYER_REGEN_DISABLED", function ()
        if state.talent.demon_blades.enabled and not state.settings.demon_blades_acknowledged and not wasWarned then
            Hekili:Notify( ns.WARNING .. L["Demon Blades cannot be forecasted.\nSee /hekili > Havoc for more information."] )
            wasWarned = true
        end
    end )
end
...
spec:RegisterSetting( "demon_blades_text", nil, {
    name = function()
        return strformat( ns.WARNING .. L["If using the %s talent, Fury gains from your auto-attacks will be forecast conservatively and updated when you actually gain resources."],
...
spec:RegisterSetting( "fel_rush_warning", nil, {
    name = strformat( L["The %1$s, %2$s, and/or %3$s talents require the use of %4$s."],
        ...
        .. strformat( L["If you do not want %s to be recommended to trigger these talents, you may want to consider a different talent build."],
        W( spec.abilities.fel_rush.name ) ) .. "\n\n"
        .. strformat( L["You can reserve %1$s charges to ensure recommendations will always leave you with charge(s) available to use, but failing to use %2$s may ultimately cost you DPS."],
        W( spec.abilities.fel_rush.name ), W( spec.abilities.fel_rush.name ) ),
    type = "description",
    width = "full",
} )
...
spec:RegisterSetting( "throw_glaive_charges_text", nil, {
    name = strformat( L["You can reserve charges of %1$s to ensure that it is always available for %2$s or |W|T1385910:0::::64:64:4:60:4:60|t |cff71d5ff%3$s (affix)|r|w procs."],
        ...
        .. strformat( L["If set to your maximum charges (2 with %1$s, 1 otherwise), %2$s will never be recommended."],
        Hekili:GetSpellLinkWithTexture( 389763 ), W( spec.abilities.throw_glaive.name ) ).. "  "
        .. strformat( L["Failing to use %s when appropriate may impact your DPS."], W( spec.abilities.throw_glaive.name ) ),
    type = "description",
    width = "full",
} )
...
spec:RegisterSetting( "retreat_warning", nil, {
    name = strformat( L["The %1$s, %2$s, and/or %3$s talents require the use of %4$s."],
        ...
        .. strformat( L["If you do not want %s to be recommended to trigger the benefit of these talents, you may want to consider a different talent build."],
        W( spec.abilities.vengeful_retreat.name ) ),
    type = "description",
    width = "full",
} )
...
spec:RegisterSetting( "retreat_and_return", "off", {
    ...
    desc = function()
        return strformat( L["When enabled, %1$s will |cFFFF0000NOT|r be recommended unless either %2$s or %3$s are available to quickly return to your current target."],
            Hekili:GetSpellLinkWithTexture( 198793 ), Hekili:GetSpellLinkWithTexture( 195072 ), Hekili:GetSpellLinkWithTexture( 232893 ) ) .. "  "
            .. strformat( L["This requirement applies to all %s and %s recommendations, regardless of talents."],
            W( spec.abilities.fel_rush.name ), W( spec.abilities.vengeful_retreat.name ) ) .. "\n\n"
            .. strformat( L["If %s is not talented, its cooldown will be ignored."],
            W( spec.abilities.felblade.name ) ) .. "\n\n"
            .. strformat( L["This option does not guarantee that %1$s or %2$s will be the first recommendation after %3$s but will ensure that either/both are available immediately."],
            W( spec.abilities.fel_rush.name ), W( spec.abilities.felblade.name ), W( spec.abilities.vengeful_retreat.name ) )
    end,
    type = "select",
    values = {
