...
if (select(2, UnitClass('player')) == 'PALADIN') then
    ...
    ns.initializeClassModule = function ()
        ...
        addHook( 'advance', function( t )
            ...
            if buff_remaining < ( PTR and 2.5 or 4 ) then return t end

            local ticks_before = math.floor( buff_remaining / ( PTR and 2.5 or 4 ) )
            local ticks_after = math.floor( max( 0, ( buff_remaining - t ) / ( PTR and 2.5 or 4 ) ) )
    ...
    if not PTR then
        ...
        addHandler( 'word_of_glory', function ()
            if buff.divine_purpose.up then removeBuff( 'divine_purpose' )
            elseif buff.the_fires_of_justice.up then removeBuff( 'the_fires_of_justice' ) end
            health.actual = min( health.max, health.actual + ( ( PTR and 1.33 or 1 ) * stat.spell_power * 8 ) )
        end )
