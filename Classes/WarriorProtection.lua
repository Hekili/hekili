...
if UnitClassBase( 'player' ) == 'WARRIOR' then
    ...
    spec:RegisterAbilities( {
        ...
        ignore_pain = {
            ...
            readyTime = function ()
                if buff.ignore_pain.up and buff.ignore_pain.v1 > 0.3 * stat.attack_power * 3.5 * ( 1 + stat.versatility_atk_mod / 100 ) and not settings.overlap_ignore_pain then
        ...
        shield_block = {
            ...
            nobuff = function()
                if not settings.stack_shield_block then
                    return "shield_block"
                    end
                end,
    ...
    spec:RegisterSetting( "overlap_ignore_pain", false, {
        name = "Overlap active |T1377132:0|t Ignore Pain",
        desc = "If checked, Ignore Pain will be recommended while it is active.",
        type = "toggle",
        width = "full"
    } )         
    
    spec:RegisterSetting( "stack_shield_block", false, {
        name = "Stack |T132110:0|t Shield Block",
        desc = "Should only be used with |T236317:0|t Reprisal to make sure you don't sit at 2 stacks of |T132110:0|t Shield Block",
        type = "toggle",
        width = "full"
    } )     
